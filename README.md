...
| [`release-please.yaml`](.github/workflows/release-please.yaml) | Cut PR-batched releases (changelog + version + tag) with [`release-please`](https://github.com/googleapis/release-please), driven by conventional commits. `target_branch` selects the branch whose release configuration and history are evaluated; it defaults to `main`. |
